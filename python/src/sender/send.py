...
def send_email_each_time(user_cursor, editor_cursor):
  ...
  if len(user_id_list) == 0 :
    return {
      'success' : False,
      'error' :  "No user in database or sendable user is not existed",
      'sendable_user_list' : {}
    }
  ...
  if len(user_interest_dict) == 0 :
    return {
      'success' : False,
      'error' : "No keyword entered by all users.",
      'sendable_user_list' : {}
    }
  ...
  if len(user_interest_article_id_dict) == 0:
    return {
      'success' : False,
      'error' : "No article ids about the keyword.",
      'sendable_user_list' : {}
    }
  ...
  return {
    'success' : True,
    'error' : "",
    'sendable_user_list' : sendable_user_info_dict
  }